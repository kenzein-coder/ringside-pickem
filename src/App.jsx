...
import { validateDisplayName, sanitizeString } from './utils/inputValidation.js';
...
import WrestlerImage from './components/WrestlerImage.jsx';
...
export default function RingsidePickemFinal() {
  ...
  const makePrediction = (eventId, matchId, winner) => {
    ...
    if (!predictionRateLimiter.isAllowed(user?.uid || 'anonymous')) {
      ...
      setLoginError(`Slow down! Please wait ${waitTime} seconds before making more predictions.`);
  ...
  if (viewState === 'onboarding') {
    return (
      <OnboardingView
        ...
        onNextPage={() => tempName.trim() && setOnboardingPage(2)}
        onBackPage={() => setOnboardingPage(1)}
        onTogglePromotion={handleOnboardingToggle}
